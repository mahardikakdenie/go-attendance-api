...
func initHandlers(db *gorm.DB, rdb *redis.Client) (*Handlers, service.CalendarCronService, service.AuthService) {
	...
	correctionService := service.NewAttendanceCorrectionService(correctionRepo, attendanceRepo, userRepo, activityRepo, rdb)
	performanceService := service.NewPerformanceService(performanceRepo, userRepo)
	superadminService := service.NewSuperadminService(superadminRepo, userRepo, roleRepo, permissionRepo, activityRepo, rdb)
	...
	calendarCronService := service.NewCalendarCronService(hrOpsRepo, userRepo, attendanceRepo, billingService)
